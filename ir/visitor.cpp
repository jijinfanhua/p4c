...
struct PushContext {
    ...
    const Visitor::Context *&stack; // 指针的引用，维护一个类似栈的结构
    ...
    PushContext(const Visitor::Context *&stck, const IR::Node *node) : stack(stck) {
        ...
        if (node->getAnnotation(IR::Annotation::debugLoggingAnnotation)) // 如果node有注释，那么启用日志记录
            Log::Detail::enableLoggingInContext = true;
        current.parent = stack;
        current.node = current.original = node;
        current.child_index = 0;
        current.child_name = "";
        current.depth = stack ? stack->depth + 1 : 1; // 当前context的深度
        assert(current.depth < 10000);  // stack overflow?
        stack = &current; // 将current上下文推入栈中，栈顶
    }
    ~PushContext() {
...
const IR::Node *Inspector::apply_visitor(const IR::Node *n, const char *name) {
    if (ctxt) ctxt->child_name = name;
    if (n && !join_flows(n)) { // 该节点不为空，并且该节点不属于join节点，继续
        PushContext local(ctxt, n); // 当前节点入栈
        auto vp = visited->emplace(n, info_t{false, visitDagOnce}); // 没有访问完，但是访问了一次
        // 如果没有插入成功，并且没有完全访问完
        if (!vp.second && !vp.first->second.done) { // vp.first 是一个迭代器，指向容器中键为 n 的元素（无论它是新插入的还是已存在的）。vp.second 是一个布尔值，指示元素是否被成功插入
            n->apply_visitor_loop_revisit(*this);
        } else if (!vp.second && vp.first->second.visitOnce) { // 如果没有插入成功， 并且访问过
            n->apply_visitor_revisit(*this);
        } else { // 插入成功；没有插入成功并且没访问过？
            vp.first->second.done = false; // 设置为没访问完
