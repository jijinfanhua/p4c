...
struct Visitor_Context {
    ...
    inline const T *findContext(const Visitor_Context *&c) const {
        c = this;
        while ((c = c->parent)) // 向上遍历其父亲，如果当前的context的Node类型是指定的类型，则将这个node进行返回，同时返回这个node对应的context。
    ...
    inline const T *findContext() const {
        ...
        return findContext<T>(c); // 从当前的context开始找，找到这个T类型的node
    }
};
...
#undef DECLARE_VISIT_FUNCTIONS
 ...
 protected:
    ...
    // 如果 joinFlows 为“true”，Visitor 将跟踪具有多个父节点的节点，并在访问该节点及其子节点之前对所有父节点中的访问者进行 flow_merge。 目前这仅适用于检查器（不适用于修改器/变换）。
...
class Inspector : public virtual Visitor {
    ...
    typedef std::unordered_map<const IR::Node *, info_t> visited_t; // 记录访问的node的信息
    std::shared_ptr<visited_t> visited; // 类型
    bool check_clone(const Visitor *) override;
...
#undef DECLARE_VISIT_FUNCTIONS
    ...
    bool visit_in_progress(const IR::Node *n) const {
        if (visited->count(n)) return !visited->at(n).done; // 如果从visited里面找到了这个节点，那么如果这个节点遍历完了，返回false，否则返回true
        return false;
    }
};
