...
class Options {
 public:
    ...
    typedef std::function<bool(const char *optarg)> OptionProcessor; // OptionProcessor是一个 bool func(const char *optarg)的函数

 protected:
    struct Option {
        cstring option; // 选项
        const char *argName;  // nullptr if argument is not required 参数名称
        const char *description; // 参数描述
        OptionProcessor processor; // 参数对应的汉书
        OptionFlags flags; // 参数选项
    };
    ...
    std::map<cstring, const Option *> options; // 选项字典
    ...
    void registerUsage(const char *msg) { additionalUsage.push_back(msg); } // 将此msg登记到additionalUsage里面
 ...
 public:
     ...
     */
    virtual std::vector<const char *> *process(int argc, char *const argv[]); // 子类需实现这个process方法；可以执行指令，并且不认识的返回到remaining里面
